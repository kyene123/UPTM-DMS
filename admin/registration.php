...
$query="insert into  registration(roomno,seater,feespm,foodstatus,stayfrom,duration,course,regno,firstName,middleName,lastName,gender,contactno,emailid,guardianName,guardianRelation,guardianContactno,corresAddress,corresCIty,corresState,corresPincode,pmntAddress,pmntCity,pmnatetState,pmntPincode,hno,password) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
$stmt = $mysqli->prepare($query);
$rc=$stmt->bind_param('siiisissssssssssisssisssiss',$roomno,$seater,$feespm,$foodstatus,$stayfrom,$duration,$course,$regno,$fname,$mname,$lname,$gender,$contactno,$emailid,$gurname,$gurrelation,$gurcntno,$caddress,$ccity,$cstate,$cpincode,$paddress,$pcity,$pstate,$ppincode,$hno,$password);
$stmt->execute();
$stmt->close();
...
$stmt1->execute();
echo"<script>alert('Students Successfully register');</script>";
	
$status=$_POST['status'];

$query="UPDATE rooms SET status=? WHERE room_no=? AND Hno=?";
$stmt= $mysqli->prepare($query);
$rc=$stmt->bind_param('sss',$status,$roomno,$hno);
$stmt->execute();
...
<div class="col-sm-8">
<select name="hno" id="hno"class="form-control"  onChange="getSeater(this.value);" onBlur="checkAvailability()" required> 
...
{
?>
<option value="<?php echo $row->hno;?>"> <?php echo $row->hno;?></option>
<?php } ?>
</select> 
...
<input type="text" name="ppincode" id="ppincode"  class="form-control" >
											...
											<div class="form-group" >
<div class="col-sm-8">
<input type="text" name="status" id="status"  class="status" value="Occupied" hidden >
